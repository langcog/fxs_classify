...
\def\iccvPaperID{2441} % *** Enter the ICCV Paper ID here
...
%%%%%%%%% TITLE
\title{Vision-based classification of developmental disorders using \textcolor{red}{gaze-movements}}
...
%One challenge is demonstrating that our features can help us better understand ASD. Another challenge is building an automated system capable of disambiguating between participants with different developmental conditions given the variability between individuals with the same disorder. 
...
%Note that for both the crafting of our features and the classifier training, we rely on modifications to existing techniques. 
...
%classifying the emotion of a person based on their eye-movements, facial expressions, or any number of interactions possible with other people. 
...
%            }
    \caption{Face Motion Analysis. Size of the circles represent the spatial variability of a keypoint over the length of an interview. (a) The interviewer with the greatest facial movement; (b) the interviewer with the least; %(c) and (d) represent the 20 px threshold radius surrounding the face key points. This threshold results in full face coverage across all interviews.
    	}
\label{fig:PERSON}
\end{figure}
...
% This justifies the use of a finer granularity of face regions to construct features as opposed to coarser features based on whether or not individuals are looking at the face.
\begin{figure}%[t]
          \subfigure[ DD]{
...
We measured the $ApEn$ of our 3 groups (DD, FXS-Female, FXS-Male). For each group $S_c$, we selected 10 random participants (about 30,000 features $\bar{f}_i$). %$Q$ contains the vectors $u$ of features of $\sim$10 participants per group. The participants are chosen randomly. 
...
\begin{figure}[h]
        ...
        \subfigure[DD vs FXS Female]{
             \includegraphics[width=0.33\textwidth]{figures/Temporal-DDvsFXSfemale-SVM}
        }
            \hfill
       \subfigure[DD vs FXS Male]{
             \includegraphics[width=0.33\textwidth]{figures/Temporal-DDvsFXSmale-SVM.jpg}
